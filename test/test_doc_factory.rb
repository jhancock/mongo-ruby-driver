...
class Person
  ...
  # if object is a Hash, the keys should be strings, not symbols
...
class TestCollection < Test::Unit::TestCase
  ...
  @@test = @@db.collection("test", lambda {|hash| Person.new(hash)})
  ...
  def test_safe_insert
    person = Person.new({"name" => "Jon"})
    @@test.insert(person.to_hash)
    @@test.insert(person.to_hash)
    assert @@db.error.include? "E11000"

    assert_raise OperationFailure do
      @@test.insert(person.to_hash, :safe => true)
    end
  end
  ...
  def test_find_one
    ...
    assert @@test.find_one().is_a? Person
    ...
    assert @@test.find_one(nil, :fields => ["hello"]).to_hash.include?("hello")
    assert !@@test.find_one(nil, :fields => ["foo"]).to_hash.include?("hello")
    assert_equal ["_id"], @@test.find_one(nil, :fields => []).to_hash.keys()
